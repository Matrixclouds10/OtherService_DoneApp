...
class _OnBoardingScreenState extends State<OnBoardingScreen> {
  ...
  void initState() {
    pageViewModelData.add(const PageViewData(
      titleText: "لخدمات الأعمال",
      subText: "لدينا الفريق الذي يضمن رضاك ويجلب ابتسامتك",
      assetsImage: Assets.imagesWorkers,
    ));

    pageViewModelData.add(const PageViewData(
      titleText: "لخدمات الأعمال",
      subText: "ستجد فريقنا يحيط بك في كل مكان ",
    ...
    sliderTimer = Timer.periodic(const Duration(seconds: 4), (timer) {
      if (currentShowIndex == 0) {
        ...
        pageController.animateTo(MediaQuery.of(context).size.width * 5,
            duration: const Duration(seconds: 1), curve: Curves.fastOutSlowIn);
          ...
          children: <Widget>[
            ...
            Padding(
              padding: EdgeInsets.symmetric(
                  vertical: kScreenPaddingNormal.h,
                  horizontal: kScreenPaddingLarge.w),
              child: CustomButton(
                title: tr(LocaleKeys.letsStart),
                color: Colors.black,
                onTap: () {
